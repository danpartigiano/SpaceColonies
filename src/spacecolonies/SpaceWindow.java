...
public class SpaceWindow {
    ...
    private int getShapeSize(Person applicant) {
        int planetIndex = colonyCalculator.getPlanetIndex(applicant.getPlanetPreference());
        if (planetIndex != - 1) {
            Planet preferred = colonyCalculator.getPlanets()[planetIndex];
            if (preferred.isQualified(applicant)) {
